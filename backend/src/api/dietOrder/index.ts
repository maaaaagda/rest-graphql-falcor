...
export const initDietOrderRoutes = (
  app: Application,
  prefix: string = ""
): void => {
  ...
  const postDietOrderController: IPostDietOrderController = container.get(
    DIET_ORDER_TYPES.IPostDietOrderController
  );
  const getDietOrderController: IGetDietOrderController = container.get(
    DIET_ORDER_TYPES.IGetDietOrderController
  );
  const updateDietOrderController: IPutDietOrderController = container.get(
  ...
  app.put(
    `${path}/:id`,
    updateDietOrderController.process.bind(updateDietOrderController)
  );
};
